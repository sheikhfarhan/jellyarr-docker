...
<details close>
   <summary>Show: VoidAuth Landing Page</summary>

![](/assets/voidauth-landingpage.png)
...
**Network:** `dockerapps-net` (Static IP: `172.20.0.37` Server / `172.20.0.38` DB) \
**URL:** `https://auth1.domain.xyz` (Public)

---

### 1. Add `voidauth` alongside [caddy compose file](/caddy/compose.yml):
...
### 2. Caddyfile: Add Snippets in Caddyfile:
...
### 3. Caddyfile: Import snippet into sites (eg: requests.domain.xyz for Jellyseerr):
...
### 4. Caddyfile: Create site for actual `voidauth` instance:
...
### 6. Add users - via "Invitations":

* Since this media server is for family and friends, I will simply create their username (I made it same as per their Jellyseerr and Jellyfin username/password).
* From the `Invitations` section, `Create Invitation` - enter username, name and group (pre-created a `Media` group).
* Copy Paste the `Invite Link` onto new browser and enter their password
* Inform them accordingly

### 7. Add ProxyAuth Domains:

* Enter the site rerquests.domain.xyz and tag `Media` group

   <details close>
      <summary>Show: ProxyAuth Domain setting</summary>

   ![](/assets/voidauth-proxyauthdomain.png)

   </details>
</br>

* Add other sites accordingly.

   <details close>
      <summary>Show: ProxyAuth Domains Lists</summary>

   ![](/assets/voidauth-proxyauthdomains-list.png)

   </details>

### 8. Add own logo and favicon in ./config/branding
